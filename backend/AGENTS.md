...
- Schema migrations are `ALTER TABLE` statements in `init_db()`, run on every start, with
  every exception swallowed (`WAIVER-OPS-001`). A failed migration is currently
